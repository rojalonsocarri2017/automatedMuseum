if (typeof AFRAME === 'undefined') {
  ...
  /**
   * Lounge collider -> detecta las colisiones entre las entidades dentro de la habitacion, Permite verificar si una entidad está colisionando con otras entidades y emitir eventos al inicio (hitstart) y al final de una colisión (hitend)
   */
  AFRAME.registerComponent('lounge-collider', {
    schema: {
      interval: {type: 'number', default: 80}, /** Define el tiempo (en milisegundos) que debe esperar el componente antes de realizar una nueva comprobación de intersección */
      objects: {type: 'selectorAll', default: ''} /** Un selector CSS que especifica qué entidades deben ser comprobadas para intersección con la entidad que tiene este componente */
    },
    ...
    tick: function (time) {
      ...
      // Emite el evento hitend cuando han dejado de  colisionar las entidades
      ...
      // Emite el evento hitstart cuando están colisionando las entidades
    ...
    /**
     * isIntersecting -> detecta si una entidad está colisionando con otra
     */
  ...
  /**
   * Lounge plinth -> crea una plataforma o pedestal rectangular en la escena, se pude personalizar el tamaño y color. Este componente escucha un evento denominado staydown, que ajusta la posición de la plataforma en el eje Y para que quede alineada con el suelo de la escena.
   */
  ...
  /**
   * Lounge staydown -> tiene como objetivo emitir un evento llamado staydown con una posición del suelo
   */
  AFRAME.registerComponent('lounge-staydown', {
    schema: {
      offsetY: { type: 'number', default: 0.15 }
    },
    ...
    floor_level: function(position) {
      ...
      this.el.emit('staydown', {worldPosition: this.el.object3D.localToWorld(localPosition)}, false);
    },
    ...
    init: function () {
      ...
      let el = this.el;
      let floorLevel = this.floor_level.bind(this);
  
      // Encontrar entidad lounge ancestro
      let ancestor = el;
      while ((ancestor = ancestor.parentNode) && !("lounge" in ancestor.attributes));
      let loungeEntity = ancestor;
  
      loungeEntity.addEventListener("loaded", () => {
        ...
        if ('data' in floorComponent) {
          this.floorY = floorComponent.data.position.y;
          floorLevel(floorComponent.data.position);
          this.tryAdjustPosition();
        } else {
          floorEntity.addEventListener("componentinitialized", (event) => {
      ...
      // Detectar carga del modelo GLTF o similar
    ...
    tryAdjustPosition: function () {
      if (this.positionAdjusted) return;
      if (this.floorY === null) return;
      if (!this.modelLoaded) return;
  
      const offset = this.data.offsetY;
      const pos = this.el.object3D.position;
      this.el.object3D.position.set(pos.x, this.floorY + offset, pos.z);
  
      this.positionAdjusted = true;
      console.log('Posición ajustada con offset en Y:', this.floorY + offset);
    },
  ...
  /**
   * diseñado para colocar una entidad en un "punto de entrada" relacionado con el componente 'lounge' por defecto
   */
  ...
  /**
   * crea y agrega un plano que representa el suelo de la escena
   */
  ...
  /**
   * crea y agrega un plano que representa el techo de la escena
   */
  ...
  AFRAME.registerComponent('lounge-wall', {
    ...
    update: function (oldData) {
      ...
      this.wall.setAttribute('src', data.texture);
  ...
  /**
   * Lounge component -> esto incluye en una escena las paredes, el techo y un punto de entrada
   */
  AFRAME.registerComponent('lounge', {
    schema: {
      ...
      ceilingTexture: {type: 'asset', default: ''},
    ...
    init: function () {
      ...
      if (this.data.ceiling) {
        this.lounge.setAttribute('lounge-ceiling', {
          ...
          'texture': this.data.ceilingTexture,
          'width': this.data.width,
          'depth': this.data.depth,
    ...
    /**
     * Devuelve la posicion del punto de entrada
     */
    entry_point() {
      var point;
